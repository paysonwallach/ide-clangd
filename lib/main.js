...
class ClangdClient extends AutoLanguageClient {
  ...
  getGrammarScopes() {
    return ['source.c', 'source.cpp', 'c', 'cpp'];
  }
  getLanguageName() {
    return 'C';
  }
  ...
  startServerProcess(projectPath) {
    ...
    const childProcess = cp.spawn(config.clangdCommand, args, {
      cwd: projectPath
    });
    ...
    childProcess.on("error", err =>
      atom.notifications.addError(
        "Unable to start the Clangd language server.", {
          ...
          description: "This can occur if you do not have Clangd installed or if it is not in your path.\n\nViewing the README is strongly recommended."
        }
      )
    );
